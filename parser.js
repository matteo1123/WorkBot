...
const arr = marketCodeParse(filepath.marketCodeStats);
const roomRevenue = arr[0];
const roomStats = arr[1];
const managerReport = managerReportParse(filepath.managerReport);
...
function parser() {
  ...
  const readstream = fs.createReadStream(filepath.trialBalance);
  readstream
    .on('error', () => {
      // handle error
    })
    ...
    .on('data', row => {
      ...
      if (row.TRX_CODE == 2501) {
        fb.rsDelivery += Number(row.TB_AMOUNT - row.TB_AMOUNT / 1.075).toFixed(
          2
        );
      ...
      const fbtaxshb = Number(fbTotal - fbTotal / 1.075).toFixed(2);
      if (taxes.FB !== fbtaxshb) {
        const adj = Number((taxes.FB - fbtaxshb).toFixed(2));
        taxes.FB = fbtaxshb;
      ...
      else if (fb.restBreak > fb.winnersCircle)
        fb.restBreak += fb.winnersCircle;
      else if (fb.restLunch > fb.winnersCircle)
        fb.restLunch += fb.winnersCircle;
